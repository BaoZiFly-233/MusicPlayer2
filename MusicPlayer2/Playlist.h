...
public:
    enum Type           //播放列表格式
    {
        ...
        PL_M3U,         //m3u播放列表
        PL_M3U8,        //m3u8播放列表
        PL_JSON         //BoTapMusic JSON歌单
    };

    CPlaylistFile();
    ~CPlaylistFile();
    bool LoadFromFile(const wstring& file_path);
    bool SaveToFile(const wstring& file_path, Type type = PL_PLAYLIST) const;
    ...
    static bool SavePlaylistToFile(const vector<SongInfo>& song_list, const wstring& file_path, Type type = PL_PLAYLIST);
