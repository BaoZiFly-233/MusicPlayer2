...
- **设备注册（`dfid`）还没做成。** 取播放地址前正规流程要先向服务端注册设备换一个 `dfid`，代码里的流程走通了，但服务端一直回 `rsa failure`。同样一套逻辑用 Node 写是能成功的，所以是我的 RSA 加密格式跟服务端预期有出入，还没找到差在哪。目前程序里 `dfid` 用的是占位符，不影响搜索、登录和波点播放；酷狗那边能不能完整播放，主要看你账号的权限。
