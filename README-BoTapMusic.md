...
**播放失败时会说明原因**，比如「需要登录酷狗概念版账号才能播放这首歌」，而不是笼统地报一个播放失败。
...
- **酷狗概念版的播放需要登录账号。** 未登录时接口会返回「需要购买」，只有 60 秒试听片段。程序里已经做了识别并给出提示，但登录功能还没做，所以目前酷狗那边搜得到、播不了。
...
- **扫码登录还没打通。** 代码里已经备好了取二维码、轮询扫码状态的接口，但酷狗的登录接口目前一律返回 `error_code 20010`（签名校验不通过）。同一组参数换成 Node 实现也是这个结果，说明不是移植出错，而是接口对签名的要求和我目前掌握的不一致，还需要继续查。因此酷狗那边目前只能搜到歌、拿不到完整播放地址。
...
MusicPlayer2/KugouSource.*       酷狗概念版音源（含扫码登录接口，尚未打通）
MusicPlayer2/OnlineSource.*      音源抽象层与注册表
MusicPlayer2/OnlineMusicDlg.*    在线音乐搜索对话框
MusicPlayer2/qrcodegen/          二维码生成库（MIT，第三方，为登录界面准备）
