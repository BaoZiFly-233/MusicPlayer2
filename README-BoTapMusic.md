...
- 首次获取播放地址前会自动做一次**设备注册**换 `dfid`（结果存在同一个 ini 里）。酷狗那边有个说法：注册拿到的 dfid 更像"网页版"的，带上它调其它接口可能过一段时间掉登录，所以程序只在取播放地址时用它，没有写进长期凭据。
...
另外 `scripts/kg_register_dev.js` 是一个独立的设备注册验证脚本（`node scripts/kg_register_dev.js`），零第三方依赖。它和程序里的 C++ 实现是同一套算法，用来单独检查设备注册接口是否正常——出问题时能快速分清是接口变了还是客户端写错了。
